...
namespace WorldMapStrategyKitDemo {
	public class DemoStandAlone : MonoBehaviour {
		...
		void FlyToCity (string cityName, string countryName) {
			map.FlyToCity (cityName, countryName, 2.0f);
		}
		...
		void TextureSample () {
			// 1st way: assign a flag texture to USA using direct API - this texture will get cleared when you call HideCountrySurfaces()
			Texture2D texture = Resources.Load<Texture2D> ("Flags/flagUSA");
			int countryIndex = map.GetCountryIndex("United States of America");
			map.ToggleCountrySurface(countryIndex, true, Color.white, texture, Misc.Vector2one, Misc.Vector2zero, 0);

			// Assign a flag texture to Brazil using decorator - the texture will stay when you call HideCountrySurfaces()
...
//		planeLocation = map.calc.toPlaneLocation;
		...
		void LocateMountPoint () {
			...
			Debug.Log ("There're " + map.mountPoints.Count + " mount point(s). You can define more mount points using the Map Editor. Mount points are stored in mountPoints.json file inside Geodata folder.");
			if (mountPointsCount > 0) {
				Debug.Log ("Locating random mount point...");
				int mp = UnityEngine.Random.Range (0, mountPointsCount - 1);
				Vector2 location = map.mountPoints [mp].unity2DLocation;
