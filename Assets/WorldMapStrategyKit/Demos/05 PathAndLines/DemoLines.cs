...
namespace WorldMapStrategyKitDemo {
	public class DemoLines : MonoBehaviour {
		...
		bool showRoutePath = false, showLinearPath = true, enableClickToMoveTank = true, showCircle = true;
		GameObjectAnimator tank;
		LineMarkerAnimator pathLine;
		GameObject circle;
		Material lineMaterialAerial, lineMaterialGround;

		float pathDrawingDuration = 0.0f;	// 0 means instant drawing.
		float pathArcElevation = 2f;			// 0 means ground-level (path will be part of the viewport texture). If you provide a value, path will be 3D.
		float pathLineWidth = 0.2f;
		float pathDashInterval = 0;
		float pathDashAnimationDuration = 0;
		float circleRadius = 1f, circleRingStart = 0, circleRingEnd = 1f;
...
//			map.FlyToLocation(mid, 0.0f, 0.1f);
		...
		void OnGUI () {
			...
			enableClickToMoveTank = GUI.Toggle (new Rect (570, 20, 200, 30), enableClickToMoveTank, "Show Circle");
			...
			GUI.backgroundColor = Color.white;
			circleRadius = GUI.HorizontalSlider(new Rect (GUIResizer.authoredScreenWidth - 190, 85, 150, 35), circleRadius, 0, 10f, sliderStyle, sliderThumbStyle);
			GUI.backgroundColor = new Color (0.1f, 0.1f, 0.3f, 0.95f);
		...
		void DropTankOnCity () {
			...
			if (tank != null) DestroyImmediate (tank);
