	...
	public partial class WMSK : MonoBehaviour {
		...
		void ReadMountPointsPackedString (string s) {
			...
			for (int k=0; k<mountPointsCount; k++) {
				...
				if (mountPointInfo.Length<6) continue;
				...
				if (countryIndex>=0) {
					...
					float x = float.Parse (mountPointInfo [4]);
					float y = float.Parse (mountPointInfo [5]);
					...
					if (mountPointInfo.Length>=7) {
						uniqueId = int.Parse(mountPointInfo[6]);
					} else {
		...
		void ReadMountPointsXML(string s) {
			...
			for (int k=0;k<mountPointsCount;k++) {
				...
				Vector2 location = new Vector2(mpJSON["X"], mpJSON["Y"]);
