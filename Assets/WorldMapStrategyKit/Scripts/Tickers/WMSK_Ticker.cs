...
namespace WorldMapStrategyKit {
	...
	public class WMSK_Ticker : MonoBehaviour {
		...
		public void Init () {
			...
			if (defaultFont == null) {
				defaultFont = GameObject.Instantiate (Resources.Load <Font> ("WMSK/Font/Lato"));
		...
		public bool UpdateTickerBands () {
			...
			for (int k=0; k<tickerBands.Length; k++) {
				...
				if (tickerBandLayer.activeSelf) {
					// position
					float posY = tickerBand.verticalOffset; // * WMSK.mapHeight;
		...
		public void AddTickerText (TickerText tickerText) {
			...
			GameObject tickerTextObj = Drawing.CreateText (tickerText.text, null, pos, customFont, tickerText.textColor, tickerText.drawTextShadow, customShadowMaterial, tickerText.shadowColor).gameObject;
