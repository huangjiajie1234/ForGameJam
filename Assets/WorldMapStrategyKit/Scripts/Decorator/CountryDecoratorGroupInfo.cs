				...
				if (countryIndex >= 0) {
					...
					if (active) {
						...
						if (decorator.isColorized) {
							map.ToggleCountrySurface (countryIndex, true, decorator.fillColor, decorator.texture, decorator.textureScale, decorator.textureOffset, decorator.textureRotation);
						...
						if (country.customLabel != null) {
							country.customLabel = null;
							//							if (country.labelGameObject != null) {	// already destroyed in RedrawMapLabels()
							//								DestroyImmediate (country.labelGameObject);
							//							}
							needsLabelRedraw = true;
						}
