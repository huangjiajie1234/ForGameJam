...
namespace WorldMapStrategyKit {
	...
	#region Public API area
		...
		public int GetMountPointIndex(int uniqueId) {
			...
			for (int k=0;k<mountPoints.Count;k++) {
				if (mountPoints[k].uniqueId==uniqueId) return k;
			}
			return -1;
		}
		...
		public List<MountPoint> GetMountPoints(  attribPredicate predicate ) {
			...
			for (int k=0;k<mountPointCount;k++) {
				...
				if (predicate(mountPoint.attrib)) selectedMountPoints.Add (mountPoint);
			}
		...
		/// </summary>
		public string GetMountPointsXMLAttributes(bool prettyPrint) {
			return GetMountPointsXMLAttributes (new List<MountPoint>(mountPoints), prettyPrint);
		}
		
		/// <summary>
		/// Gets XML attributes of provided mount points in jSON format.
		/// </summary>
		public string GetMountPointsXMLAttributes(List<MountPoint> mountPoints, bool prettyPrint) {
		...
		/// </summary>
		public void SetMountPointsXMLAttributes(string jSON) {
