...
namespace WorldMapStrategyKit
{
	...
	public partial class WMSK : MonoBehaviour
	{
		...
		public string provinceAttributeFile {
			...
			set { if (value!=_provinceAttributeFile) {
					_provinceAttributeFile = value;
					if (_provinceAttributeFile == null) _provinceAttributeFile = PROVINCE_ATTRIB_DEFAULT_FILENAME;
					isDirty = true;
	...
	#region Public API area
			...
			if (countryIndex>=0) {
				...
				for (int p=0; p<country.provinces.Length; p++) {
					...
					for (int pr=0;pr<province.regions.Count;pr++) {
						if (province.regions [pr].ContainsPoint (localPosition)) {
			...
			if (hits.Length > 0) {
				for (int k=0; k<hits.Length; k++) {
					if (hits [k].collider.gameObject == gameObject) {
						...
						return GetProvinceRegionIndex(localHit, out provinceIndex, out regionIndex);
					}
				}
			}
			provinceIndex = -1;
		...
		public Province GetProvince (string provinceName, string countryName)
		{
			...
			if (provinceIndex>=0) return provinces[provinceIndex];
			return null;
		}
		...
		public int GetProvinceIndex(int uniqueId) {
			...
			for (int k=0;k<provinceCount;k++) {
				if (_provinces[k].uniqueId==uniqueId) return k;
			}
			return -1;
		}
		...
		{
			ToggleProvinceSurface(provinceIndex, visible, color, null, Misc.Vector2one, Misc.Vector2zero, 0);
//			if (!visible) {
...
//				ToggleProvinceRegionSurface (provinceIndex, r, visible, color);
		...
		/// </summary>
		public void ToggleProvinceSurface (int provinceIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation)
		{
			if (!visible) {
				HideProvinceSurfaces (provinceIndex);
				return;
			...
			for (int r=0; r<province.regions.Count; r++) {
				if (r==province.mainRegionIndex) {
					ToggleProvinceRegionSurface (provinceIndex, r, visible, color, texture, textureScale, textureOffset, textureRotation);
				} else {
					ToggleProvinceRegionSurface (provinceIndex, r, visible, color, null, Misc.Vector2one, Misc.Vector2zero, 0);
				}
			}
		}
		...
		{
			ToggleProvinceRegionSurface (provinceIndex, regionIndex, visible, color,  null, Misc.Vector2one, Misc.Vector2zero, 0);
		}

		/// <summary>
		/// Colorize specified region of a country by indexes.
		/// </summary>
		public void ToggleProvinceRegionSurface (int provinceIndex, int regionIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation)
		{
			if (!visible) {
			...
			bool isHighlighted = provinceHighlightedIndex == provinceIndex && (provinceRegionHighlightedIndex == regionIndex || _highlightAllProvinceRegions)  && _enableProvinceHighlight;
			if (surf != null) {
				if (!surf.activeSelf)
					surf.SetActive (true);
				...
				surf = GenerateProvinceRegionSurface (provinceIndex, regionIndex, surfMaterial, textureScale, textureOffset, textureRotation);
		...
		public List<Province> GetProvinces(  attribPredicate predicate ) {
			...
			for (int k=0;k<provinceCount;k++) {
				...
				if (predicate(province.attrib)) selectedProvinces.Add (province);
			}
		...
		/// </summary>
		public string GetProvincesXMLAttributes(bool prettyPrint) {
			return GetProvincesXMLAttributes (new List<Province>(provinces), prettyPrint);
		}
		
		/// <summary>
		/// Gets XML attributes of provided provinces in jSON format.
		/// </summary>
		public string GetProvincesXMLAttributes(List<Province> provinces, bool prettyPrint) {
			JSONObject composed = new JSONObject();
			int provinceCount = provinces.Count;
			for (int k=0;k<provinceCount;k++) {
				Province province = _provinces[k];
				if (province.attrib.keys!=null) composed.AddField(province.uniqueId.ToString(), province.attrib);
			}
		...
		/// </summary>
		public void SetProvincesXMLAttributes(string jSON) {
