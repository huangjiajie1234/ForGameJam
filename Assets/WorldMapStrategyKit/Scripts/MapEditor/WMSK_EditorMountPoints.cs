...
//			if (!mp.attrib.HasField(GUIMountPointNewTagKey)) {
//				mp.attrib[GUIMountPointNewTagKey] = GUIMountPointNewTagValue;
//				GUIMountPointNewTagKey = "";
//				GUIMountPointNewTagValue = "";
//			    mountPointChanges = true;
//				return true;
...
//		}
		...
		public string GetMountPointsGeoData () {
			...
			for (int k=0;k<map.mountPoints.Count;k++) {
				...
				if (mp.provinceIndex>=0 && mp.provinceIndex<map.provinces.Length) provinceUniqueID = map.provinces[mp.provinceIndex].uniqueId;
				mpJSON.AddField("Province", provinceUniqueID);
				int countryUniqueID = -1;
				if (mp.countryIndex>=0 && mp.countryIndex<map.countries.Length) countryUniqueID = map.countries[mp.countryIndex].uniqueId;
		...
		void PopulateRegion(int countryIndex, int provinceIndex, Region region) {
			...
			while(mountPoints>0) {
				...
				for (int k=0;k<100;k++) {
					...
					if (region.ContainsPoint(point) && map.GetMountPointNearPoint(point, r)<0) {
