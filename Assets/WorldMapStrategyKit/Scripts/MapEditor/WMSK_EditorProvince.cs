...
namespace WorldMapStrategyKit_Editor {
	...
	public partial class WMSK_Editor : MonoBehaviour {
		...
		public bool provinceAttribChanges; // if there's any pending change to be saved
		...
		/// <summary>
		/// Delete all provinces from current country. Called from DeleteCountry.
		/// </summary>
		void DeleteProvinces() {
			map.ProvincesDelete(countryIndex);
			provinceChanges = true;
		}
		...
		public void ProvinceCreate() {
			...
			Province newProvince = new Province("New Province" + (provinceIndex+1).ToString(), countryIndex, map.GetUniqueId(new List<IExtendableAttribute>(map.provinces)));
		...
		public void ProvinceTransferTo() {
			...
			if (pc.OverlapsSubjectAndClipping()) {
				...
				newCountryRegion.points = new List<Vector2>(provinceRegion.points).ToArray();
