...
namespace WorldMapStrategyKit_Editor {
	...
	public class WMSKDecoratorInspector : Editor {
		...
		WMSK _map { get { return _decorator.map; } }
...
//				_decorator.Init ();
		...
		public override void OnInspectorGUI () {
			...
			if (GUILayout.Button ("Clear")) {
				...
				if (decorator!=null) decorator.isNew = true;
				ReloadGroupNames ();
				_decorator.ReloadCountryNames ();
			}
		...
		void ToggleZoomState () {
			zoomState = !zoomState;
			_map.transform.rotation = Camera.main.transform.rotation;
			if (zoomState) {
				oldMapPosition = _map.transform.position;
				oldMapRotation = _map.transform.rotation;
				_map.transform.position = Camera.main.transform.position + Camera.main.transform.forward * WMSK.mapWidth * 0.1f;
				int countryIndex = _map.GetCountryIndex(_decorator.GUICountryName);
				if (countryIndex>=0) {
					Vector2 center = _map.countries[countryIndex].center;
					_map.transform.Translate(new Vector2(-center.x * WMSK.mapWidth, -center.y * WMSK.mapHeight));
				}
			} else {
